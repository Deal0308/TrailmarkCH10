...
Trailmark is one continuous iPhone and Apple Watch project. The iPhone app has **Today**, **Field Journal**, **Recovery**, **Workout**, and **Journeys** tabs. The existing watch target has four pages: **Wrist Home**, **Voice Memos**, **Live Vitals**, and **Motion**, with the walking workout reachable from Home and Vitals. Health access is optional; opening the watch app never waits for a Health connection.

The current submission report is **[Course 2 final report: TrailMark on the wrist](docs/WatchFinalReport.md)**, covering all four watch features, quantified reuse, design, sampling cost, and remaining device evidence. The [Course 1 final report](docs/FinalReport.md) covers the iPhone features. See also [Recovery reflection](docs/RecoveryAssignment.md), [Wrist Home](docs/WatchHomeAssignment.md), [Wrist Memo](docs/WatchMemoAssignment.md), and [Live Vitals](docs/WatchVitalsAssignment.md).

## Current delivery status

All four watch features remain in the same source project and existing watch target. **The redesigned watchOS device and watchOS Simulator builds, including the iPhone companion, passed on September 22, 2026.** No app or simulator was launched for this visual revision, so its runtime layout review remains pending. The earlier Apple Watch Series 11 (46mm), watchOS 26.5 simulator check confirmed all four pages were reachable without Health authorization **before the redesign**. Automated tests, microphone recording/playback, physical-watch sensor evidence, submission screenshots, and demo recordings remain outstanding.
...
Motion adds `MotionSnapshot`, `MotionProviding`, `MotionService`, and `MotionViewModel` to the shared package, plus the wrist-specific `WatchMotionView`. No Core Motion code or HealthKit code is implemented in app views. The [Course 2 final report](docs/WatchFinalReport.md) inventories **18 package files reused by both apps: 17 existing model/service/state/support files plus one shared design file**. It distinguishes watch-focused package extensions and the four portable Motion files, and explains the limits of historical unchanged-code claims. The report includes the named watchOS design justification, RMS calculation, 10 Hz/2 Hz sampling tradeoff, and device-demo checklist.
